...
def save_test_results(modelName, dataName, results):
    filename = modelName + '_' + dataName + '_testing_results.txt'
    with open(filename, 'w') as fp:
        videoName = ""
        for item in results:
            if type(item) == str:
                videoName = videoName + item
                print(videoName)
            if type(item) != str:
                fp.write("%s\n" % videoName)
...
def pickle_test_results(modelName, dataName, results):
    filename = modelName + '_' + dataName + '_testing_results.pkl'
...
#             train_map, train_loss = train_step(model, gpu, optimizer, dataloader['train'], epoch)
            ...
            if best_map < val_map:
                best_map = val_map
                torch.save(model.state_dict(),'Datasets/TrainedModel/'+str(args.model)+'/weight_epoch_'+str(args.lr)+'_'+str(epoch))
                # torch.save(model,'/Datasets/TrainedModel/'+str(args.model)+'/model_epoch_'+str(args.lr)+'_'+str(epoch))
                # torch.save(model,'/Datasets/TrainedModel/'+str(args.model)+'/model_epoch_'+str(args.lr))
                torch.save(model, 'Datasets/TrainedModel/'+str(args.model)+'/model_'+str(label_model_name(args.model, args.dataset)))
                print('save here:','/Datasets/TrainedModel/'+str(args.model)+'/weight_epoch_'+str(args.lr)+'_'+str(epoch))
...
def eval_model(model, dataloader, baseline=False):
    ...
    for data in dataloader:
        ...
        #data[2].numpy()[0] is negative
        #probs.data.cpu().numpy()[0] is positive
        
        results[other[0][0]] = (outputs.data.cpu().numpy()[0], probs.data.cpu().numpy()[0], data[2].numpy()[0], fps)
        
        listOfData = list(results.items())
#         print(results)
...
#                 f.write('%s:%s\n' % (key, value))
    ...
    save_test_results(args.model, args.mode, listOfData[0])
    pickle_test_results(args.model, args.mode, results)
#     print(listOfData[0])
    return results
